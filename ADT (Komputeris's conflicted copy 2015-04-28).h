...
namespace Four20 {
	...
	class Proc : public Obj { // procedure (also used for the main program)
		vector<Obj*> locals;      // local objects declared in this procedures
		int nextAdr;	         // next free address in this procedure
		Proc* program = NULL;	         // link to the Proc node of the main program or null
		Parser* parser;         // for error messages

		Proc(wchar_t* name, int type, Proc* program, Parser* parser) : Obj(name, type) {
